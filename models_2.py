...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        elif mdef["type"] == "stage1":
            modules.add_module(f"ghostnet_stage1", stage1())
            filters = int(mdef["filters"])
        elif mdef["type"] == "stage2":
            modules.add_module(f"ghostnet_stage2", stage2())
            filters = int(mdef["filters"])
        elif mdef["type"] == "stage3":
            modules.add_module(f"ghostnet_stage3", stage3())
            filters = int(mdef["filters"])
        elif mdef["type"] == "stage4":
            modules.add_module(f"ghostnet_stage4", stage4())
            filters = int(mdef["filters"])
        elif mdef["type"] == "stage5":
            modules.add_module(f"ghostnet_stage5", stage5())
            filters = int(mdef["filters"])
...
if __name__ == "__main__":
    ...
    inputs = torch.randn(4, 3, 416, 416).to(device)

    model = Darknet('cfg/yolov3-ghost.cfg', '416').to(device)
    print(model)
